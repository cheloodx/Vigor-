...
import { ArrowLeft, Car, Loader } from 'lucide-react'
...
const FALLBACK: Record<string, { desc: string; check: string; cost: string }> = {
...
export default function DiagramaAuto() {
  ...
  const [aiInfo, setAiInfo] = useState<{ desc: string; check: string; cost: string } | null>(null)

  const selectPart = async (id: string) => {
    setSelected(id)
    const partName = PARTS.find(p => p.id === id)?.name || id
    if (!make) { setAiInfo(FALLBACK[id] || null); return }
    setLoading(true)
    try {
      const res = await api.chat(
        `Componenta "${partName}" la un ${make}. Ofera: 1) Descriere scurta specifica acestei masini, 2) Interval verificare in km, 3) Cost estimat reparatie in EUR. Raspunde in romana, format: descriere|interval|cost`,
        make
      )
      const parts = res.response.split('|')
      if (parts.length >= 3) {
        setAiInfo({ desc: parts[0].trim(), check: parts[1].trim(), cost: parts[2].trim() })
      } else {
        setAiInfo({ desc: res.response.slice(0, 200), check: FALLBACK[id]?.check || 'Conform manual', cost: FALLBACK[id]?.cost || 'Variabil' })
      }
    } catch {
      setAiInfo(FALLBACK[id] || null)
    }
    setLoading(false)
  }
    ...
    <div className="min-h-screen pt-20">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div><h1 className="text-xl font-bold text-white">Diagrama Auto</h1><p className="text-xs text-slate-500">Componente + AI diagnostic</p></div>
        </div>
        <div className="glass-card rounded-2xl p-4 mb-4">
          <input value={make} onChange={e => setMake(e.target.value)} placeholder="Marca + Model (ex: BMW E46) pentru info AI" className="w-full glass rounded-xl px-4 py-3 text-white text-sm" />
        </div>
        <div className="glass-card rounded-2xl p-6 mb-6">
          <p className="text-sm text-slate-400 mb-4">Apasa pe o componenta pentru detalii AI:</p>
          <div className="relative w-full aspect-[2/1] bg-gradient-to-b from-slate-800/50 to-slate-900/50 rounded-xl overflow-hidden border border-slate-700/50">
            ...
            {PARTS.map(p => (
              <button key={p.id} onClick={() => selectPart(p.id)} className={`absolute w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all hover:scale-125 ${selected === p.id ? 'ring-2 ring-white scale-125' : ''}`}
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {PARTS.map(p => (
              <button key={p.id} onClick={() => selectPart(p.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selected === p.id ? 'bg-white/20 text-white' : 'glass text-slate-400 hover:text-white'}`}>
                {p.name}
              </button>
            ))}
          </div>
        ...
        {loading && (
          <div className="glass-card rounded-2xl p-6 text-center"><Loader className="w-6 h-6 text-cyan-400 animate-spin mx-auto" /><p className="text-xs text-slate-500 mt-2">AI analizeaza componenta...</p></div>
        )}
        {info && selected && !loading && (
          <div className="glass-card rounded-2xl p-6 border-l-4" style={{ borderColor: PARTS.find(p => p.id === selected)?.color }}>
            <h3 className="text-lg font-bold text-white mb-2">{PARTS.find(p => p.id === selected)?.name} {make && <span className="text-xs text-cyan-400 font-normal">({make})</span>}</h3>
            <p className="text-sm text-slate-300 mb-4">{info.desc}</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="glass rounded-xl p-3"><p className="text-[10px] text-slate-500 uppercase">Interval verificare</p><p className="text-sm font-bold text-white">{info.check}</p></div>
              <div className="glass rounded-xl p-3"><p className="text-[10px] text-slate-500 uppercase">Cost estimat reparatie</p><p className="text-sm font-bold text-white">{info.cost}</p></div>
            </div>
          </div>
        )}
      </div>
